...
var w = 0;
var l = 0;
var table =n document.getElementById("stats_table");
for(var rN = 2; rN <table.rows.length; rN++)
{
	var r = table.rows[rN];
	var buffScore = r.cells[2].innerHTML;
	var oppScore = r.cells[3].innerHTML;

	if(buffScore > oppScore)
	{
		r.cells[4].innerHTML += "CU Boulder";
		w += 1;
	}
	else {
		r.cells[4].innerHTML += r.cells[1].innerHTML;
		l += 1;
	}
}
...
function loadPlayersPage(){
	...
	{
		var p = players[i].name;
		in = in + "<a class = 'dropdown-item' href ='#' onclick = 'switchPlayers("+ i +")'>"+p
		+"</a>"
	}
	pMenu.innerHTML = in;
}
function switchPlayers(playerNum)
{
	...
	var pass = Math.floor(player[playerNum].pass_yards/players[playerNum].games_played);
	var run = Math.floor(player[playerNum].rushing_yards/players[playerNum].games_played);
	var rec = Math.floor(player[playerNum].receiving_yards/players[playerNum].games_played);
	document.getElementById("avg_p_yards").innerHTML = pass;
	document.getElementById("avg_r_yards").innerHTML = run;
